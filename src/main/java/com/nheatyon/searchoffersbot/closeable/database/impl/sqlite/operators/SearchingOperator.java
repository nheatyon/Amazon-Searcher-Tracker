...
public final class SearchingOperator extends DatabaseConcreteExecutor implements DatabaseOperator {

    public SearchingOperator(ConfigurationManager<?, String> config, OperatorFactory operators) {
        super(config, operators);
    }

    @Override
    public void set(String userId, Object... values) {
        executeUpdate("UPDATE " + getTableName() + " SET is_searching=? WHERE user_id=?;", true, values[0], userId);
    }

    @Override
    @SuppressWarnings("unchecked")
    public <T> T get(String... data) {
        return (T) Boolean.valueOf((int) executeQuery("SELECT is_searching FROM " + getTableName() + " WHERE user_id=?;", data[0]).get(0) == 1);
    }
}
